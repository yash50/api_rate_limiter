...
Approach taken:
First thing to be noted is that only authenticated users are allowed to make any calls to the APIs. If an unauthenticated user makes a request to any of the APIs, the request will be declined. Also, a token needs to be generated manually for each user once registered, which is used for making any further requests for the APIs.
